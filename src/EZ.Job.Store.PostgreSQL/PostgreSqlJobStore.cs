...
public sealed class PostgreSqlJobStore : IJobStore
{
    ...
    public PostgreSqlJobStore(string connectionString)
    {
    ...
    public async ValueTask<Job?> GetAsync(string id, CancellationToken cancellationToken = default)
    {
        ...
        await using var cmd = new NpgsqlCommand("SELECT * FROM ez_jobs WHERE id = @id", conn);
    ...
    public async ValueTask<IEnumerable<Job>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        ...
        await using var cmd = new NpgsqlCommand("SELECT * FROM ez_jobs ORDER BY created_at", conn);
    ...
    public async ValueTask<IEnumerable<Job>> GetPendingAsync(CancellationToken cancellationToken = default)
    {
        ...
        await using var cmd = new NpgsqlCommand("SELECT * FROM ez_jobs WHERE status = 0 ORDER BY created_at", conn);
    ...
    private static Job ReadJob(NpgsqlDataReader reader)
    {
        return new Job(
